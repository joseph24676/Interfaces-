...
document.getElementById('register_email').addEventListener('input', function() {
    ...
    if (!/@/.test(email)) {
        document.getElementById('email_error').innerText = 'Debe incluir "@"';
...
function registerUser() {
    ...
    document.getElementById('register_username').addEventListener('input', function(event) {
        console.log('Input event triggered'); // Verificar si el evento se activa
        const cedula = event.target.value;
        const cedulaError = document.getElementById('cedula_error');
        
        if (!/^\d*$/.test(cedula)) {
            cedulaError.innerText = 'Solo se permiten números';
    ...
    if (valid && name && email && cedula && password) {
